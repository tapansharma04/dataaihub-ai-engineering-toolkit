...
def render_text_report(report: AnalysisReport) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Note: Findings highlight corpus characteristics that may affect AI/RAG systems.",
...
def _render_finding(finding: Finding) -> list[str]:
    ...
    if sample:
        ...
        total_affected = finding.evidence.get("affected_document_count")
        if not isinstance(total_affected, int):
            total_affected = len(finding.affected_documents)
        remaining = max(total_affected - min(5, len(sample)), 0)
        suffix = f" (+{remaining} more)" if remaining > 0 else ""
        lines.extend(
            [
                "",
                f"      Affected documents: {total_affected}",
