"""Normalize OpenAI observations into canonical ModelRecord values."""
...
from samyak.model.providers.openai.errors import OpenAIAdapterError
from samyak.model.providers.openai.observations import OpenAILifecycle, OpenAIModelObservation
...
_SOURCE_RANK = {
    OpenAISourceType.DEPRECATIONS: 3,
    OpenAISourceType.MODEL_PAGE: 2,
    OpenAISourceType.MODELS_INDEX: 1,
}
...
_LIFECYCLE = {
    OpenAILifecycle.ACTIVE: LifecycleState.ACTIVE,
    OpenAILifecycle.DEPRECATED: LifecycleState.DEPRECATED,
    OpenAILifecycle.RETIRED: LifecycleState.RETIRED,
}


def normalize_openai_observations(
    observations: Sequence[OpenAIModelObservation],
    *,
    verified_at: str,
) -> tuple[ModelRecord, ...]:
    """Map OpenAI observations to canonical records. Does not invent facts."""
    if not observations:
        raise OpenAIAdapterError("OpenAI observations are missing")
    try:
        validate_timestamp("verified_at", verified_at)
    except CatalogValidationError as exc:
        raise OpenAIAdapterError("verified_at must be a timezone-aware ISO-8601 timestamp") from exc

    grouped: dict[str, list[OpenAIModelObservation]] = defaultdict(list)
    ...
    try:
        ...
        raise OpenAIAdapterError("OpenAI observations could not be normalized") from exc
    return tuple(records)


def _record_for(
    model_id: str,
    group: Sequence[OpenAIModelObservation],
    *,
    verified_at: str,
    ...
    page_present = any(item.source.source_type is OpenAISourceType.MODEL_PAGE for item in group)
    deprecations_present = any(
        item.source.source_type is OpenAISourceType.DEPRECATIONS for item in group
    )
    ...
    if not deprecations_present:
        # Dates and replacements were not evaluated from deprecations.
...
class _DeprecationEvent:
    """One OpenAI deprecation-table row, treated as a historical lifecycle event.

    Internal to this adapter. Generic ``_merge()`` never sees this type; it
    receives the observation for the selected current event (or competing
    current events when the table is ambiguous).
    """
...
def _observations_for_lifecycle_fields(
    ...
    """Replace historical deprecation rows with the current event(s).

    Capability and identity fields keep the original group. Lifecycle,
    ``deprecated_at``, ``retirement_at``, and replacement are reduced together
    so one event cannot be split into unrelated claims.
    """
    deprecation = [
        item for item in group if item.source.source_type is OpenAISourceType.DEPRECATIONS
    ]
    others = [
        item for item in group if item.source.source_type is not OpenAISourceType.DEPRECATIONS
    ]
...
def _current_deprecation_observations(
    ...
    """Select the current OpenAI deprecation event(s) for one model id.

    Rules:
    - ``deprecation_listing`` distinguishes upcoming (current) from past
      (historical). Upcoming rows beat past rows even if a past announcement
      date is later.
    - One selected event keeps lifecycle, dates, and replacements linked.
    - Multiple distinct upcoming events are competing current facts: keep them
      all so generic merge can conflict. Do not last-write-wins by parse order
      or by independently taking latest dates/replacements.
    - Multiple past events: take the unique latest announcement as one event.
      If announcement dates cannot uniquely identify one event, fail closed.
    """
    events = tuple(_DeprecationEvent(item) for item in observations)
    upcoming = _unique_events(event for event in events if event.listing == "upcoming")
    past = _unique_events(event for event in events if event.listing == "past")
    unlabeled = _unique_events(
        event for event in events if event.listing not in {"upcoming", "past"}
    )
    if unlabeled:
        return tuple(event.observation for event in _unique_events(events))
    if upcoming:
        return tuple(event.observation for event in upcoming)
    if not past:
        return tuple(event.observation for event in _unique_events(events))
...
def _unique_latest_past(events: Sequence[_DeprecationEvent]) -> _DeprecationEvent | None:
    """Return the unique latest past event, or None when recency is ambiguous."""
...
def _merged_resolves_to(group: Sequence[OpenAIModelObservation], verified_at: str) -> str | None:
...
def _lifecycle_fact(
    group: Sequence[OpenAIModelObservation],
    verified_at: str,
    fallback: OpenAIModelObservation,
) -> Fact[LifecycleState]:
    """Resolve lifecycle from explicit evidence only.

    Deprecation and retirement observations win. An index listing may
    establish ACTIVE. Silence on a model page is UNKNOWN, not ACTIVE.
    Absence from the index is not RETIRED.
    """
    stated = _merge(
        group,
...
def _reject_contradictory_dates(
    ...
    if retirement_at.value < deprecated_at.value:
        raise OpenAIAdapterError(
            f"OpenAI documentation for {model_id} has retirement_at earlier than deprecated_at"
        )


def _lifecycle_value(item: OpenAIModelObservation, verified_at: str) -> LifecycleState | None:
    if item.lifecycle is None:
        return None
    if (
        item.lifecycle is OpenAILifecycle.DEPRECATED
        and item.retirement_at is not None
        and _date_on_or_before(item.retirement_at, verified_at)
    ):
...
def _merge(
    group: Sequence[OpenAIModelObservation],
    extract: Callable[[OpenAIModelObservation], Any],
    verified_at: str,
    fallback: OpenAIModelObservation,
    *,
    field: str | None = None,
    ...
    stated: list[tuple[OpenAIModelObservation, Any]] = []
    for item in group:
        value = extract(item)
    ...
    if not stated:
        ...
        if field in _LIFECYCLE_FIELDS and not any(
            item.source.source_type is OpenAISourceType.DEPRECATIONS
    ...
    distinct: list[tuple[OpenAIModelObservation, Any]] = []
    seen: list[Any] = []
...
def _fallback_observation(
    group: Sequence[OpenAIModelObservation], *, page_present: bool
) -> OpenAIModelObservation:
    if page_present:
        return next(
            item for item in group if item.source.source_type is OpenAISourceType.MODEL_PAGE
        )
    ranked = sorted(group, key=lambda item: _SOURCE_RANK[item.source.source_type], reverse=True)
    return ranked[0]


def _alias_source(group: Sequence[OpenAIModelObservation], alias_id: str) -> OpenAIModelObservation:
    for item in group:
...
def _provenance(
    ...
    *,
    ...
    if confidence is None:
        confidence = (
            Confidence.MEDIUM
            if item.source.source_type is OpenAISourceType.MODELS_INDEX
