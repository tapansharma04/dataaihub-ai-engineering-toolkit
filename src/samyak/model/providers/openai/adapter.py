"""Assemble a catalog from captured OpenAI documentation. No network access."""
...
from samyak.model.providers.openai.sources import CapturedSource
from samyak.model.records import RECORD_FACT_NAMES, ModelRecord


def catalog_from_openai_sources(
    ...
    *,
    ...
    """Parse, normalize, and validate captured OpenAI documentation.

    ``overlay`` is catalog metadata only. This function does not write a bundled
    snapshot or a cache file.
    """
    observations = parse_openai_sources(sources)
    records = normalize_openai_observations(observations, verified_at=verified_at)
